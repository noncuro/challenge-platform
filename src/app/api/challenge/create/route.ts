...
// get the user's email from the body. Generate a random token and return it in the response.
...
export async function POST(req: Request, res: Response) {
    ...
    const { email, duration, challengeDescription } = await req.json() as CreateChallengeRequest; // The user's email for the challenge
    ...
    if (existingStatus) {
        return new Response(JSON.stringify({ error: 'Challenge already exists for this email' }), { status: 400 });
    }
    ...
    // Set the challenge status in Redis
    await redisClient.set(`challenge:${email}`, JSON.stringify(newChallengeStatus));
